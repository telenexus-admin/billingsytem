...
try {
    ...
    // Verify token (valid for current hour)
    $expected_token = md5($router['password'] . $router['ip_address'] . date('YmdH'));
...
function queryMikroTikRouter($router)
{
    ...
    try {
        // Check if MikroTik class exists
        if (!class_exists('Mikrotik')) {
            return [
                'success' => false,
                'error' => 'MikroTik API class not available',
        ...
        // Set socket timeout if method exists
