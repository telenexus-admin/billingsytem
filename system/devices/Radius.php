...
class Radius
{
     ...
     * @param array $plan
     * @param string|null $expiration If set (e.g. from recharge), "Y-m-d H:i:s" written to radcheck; otherwise computed from plan validity
     */
    function add_customer($customer, $plan, $expiration = null)
    {
        ...
        if ($expiration !== null && trim((string) $expiration) !== '') {
            $ts = strtotime(trim($expiration));
            if ($ts !== false) {
                $date_exp = date('Y-m-d', $ts);
                $time = date('H:i:s', $ts);
            } else {
                $parts = preg_split('/\s+/', trim($expiration), 2);
                $date_exp = $parts[0];
                $time = isset($parts[1]) ? $parts[1] : '23:59:59';
            }
    ...
    function sync_customer($customer, $plan)
    {
        ...
        $this->customerAddPlan($customer, $plan, $t['expiration'] . ' ' . $t['time']);
        return true;
    }
    ...
    public function change_username($plan, $from, $to)
    {
        ...
        $c = $this->getTableCustomerAttr()->where_equal('username', $from)->findMany();
        if ($c) {
            foreach ($c as $u) {
                $u->username = $to;
                $u->save();
    ...
    {
        global $radius_pass;
        $salt = isset($radius_pass) ? $radius_pass : '';
        ...
        if ($r) {
            ...
            $r->value = md5(time() . $username . $salt);
            $r->save();
            if ($radiusDisconnect) {
    ...
    public function customerAddPlan($customer, $plan, $expired = '')
    {
        ...
        if ($this->customerUpsert($customer, $plan)) {
            ...
            if ($expired != '') {
                ...
                // session timeout [it reset everyday, am still making my research] we can use it for something like 1H/Day - since it reset daily, and Max-All-Session clear everything
                //$this->upsertCustomer($customer['username'], 'Session-Timeout', 3600); // 3600 = 1 hour
                $expTs = strtotime($expired);
                if ($expTs !== false) {
                    $remain = $expTs - time();
                    if ($remain < 0) {
                        $remain = 0;
                    }
                    $this->upsertCustomer($customer['username'], 'Max-All-Session', $remain);
                    $this->upsertCustomer($customer['username'], 'Expiration', date('d M Y H:i:s', $expTs));

                    // Mikrotik Spesific
                    $this->upsertCustomer(
                        $customer['username'],
                        'WISPr-Session-Terminate-Time',
                        date('Y-m-d', $expTs) . 'T' . date('H:i:s', $expTs) . Timezone::getTimeOffset($config['timezone'])
                    );
                }
            } else {
                $this->delAtribute($this->getTableCustomer(), 'Max-All-Session', 'username', $customer['username']);
    ...
    public function disconnectCustomer($username)
    {
         ...
         */
        $act = $this->getTableAcct()->where_raw("acctstoptime IS NULL")->where('username', $username)->find_one();
        if (!$act || empty($act['nasipaddress'])) {
            return [];
        }
        ...
        $result = [];
        $framedIp = isset($act['framedipaddress']) ? $act['framedipaddress'] : '';
        foreach ($nas as $n) {
            $port = 3799;
            if (!empty($n['ports'])) {
                $port = $n['ports'];
            }
            $result[] = $n['nasname'] . ': ' . @shell_exec("echo 'User-Name = $username,Framed-IP-Address = " . $framedIp . "' | radclient -x " . trim($n['nasname']) . ":$port disconnect '" . $n['secret'] . "'");
        }
        return $result;
    }
