...
switch ($action) {
    ...
    case 'hotspot':
        ...
        $ui->assign('devices', Package::servicePlanDeviceOptions($DEVICE_PATH));

        $hotspotListBase = function () {
        ...
        $applyHotspotListFilters = function ($q, $withStatusFilter) use ($type1, $type2, $type3, $bandwidth, $valid, $router, $device, $name, $status) {
            if (!empty($type1)) {
                $q->where('tbl_plans.prepaid', $type1);
            }
            if (!empty($type2)) {
                $q->where('tbl_plans.plan_type', $type2);
            }
            if (!empty($type3)) {
                $q->where('tbl_plans.typebp', $type3);
            }
            if (!empty($bandwidth)) {
                $q->where('tbl_plans.id_bw', $bandwidth);
            }
            if (!empty($valid)) {
                $q->where('tbl_plans.validity_unit', $valid);
            }
            if (!empty($router)) {
                if ($router == 'radius') {
                    $q->where('tbl_plans.is_radius', '1');
                } else {
                    $q->where_raw("(FIND_IN_SET(?, tbl_plans.routers) OR tbl_plans.routers = 'all')", [$router]);
                }
            }
            if (!empty($device)) {
                $q->where('tbl_plans.device', $device);
            }
            if ($withStatusFilter && in_array($status, ['0', '1'])) {
                $q->where('tbl_plans.enabled', $status);
            }
            if ($name != '') {
                $q->where_like('tbl_plans.name_plan', '%' . $name . '%');
            }
        };

        $qTotal = $hotspotListBase();
        $applyHotspotListFilters($qTotal, true);
        $hotspot_stats_total = $qTotal->count();

        $qEnabled = $hotspotListBase();
        $applyHotspotListFilters($qEnabled, false);
        $qEnabled->where('tbl_plans.enabled', '1');
        $hotspot_stats_enabled = $qEnabled->count();

        $qDisabled = $hotspotListBase();
        $applyHotspotListFilters($qDisabled, false);
        $qDisabled->where('tbl_plans.enabled', '0');
        $hotspot_stats_disabled = $qDisabled->count();

        $qExpiredPkg = $hotspotListBase();
        $applyHotspotListFilters($qExpiredPkg, false);
        $qExpiredPkg->where_raw('tbl_plans.plan_expired IS NOT NULL AND tbl_plans.plan_expired > 0');
        $hotspot_stats_expired_package = $qExpiredPkg->count();

        $hotspot_list_per_page = 20;
        $hotspot_list_page = (int) _get('p', 1);
        if ($hotspot_list_page < 1) {
            $hotspot_list_page = 1;
        }
        $hotspot_stats_from = $hotspot_stats_total > 0 ? (($hotspot_list_page - 1) * $hotspot_list_per_page + 1) : 0;
        $hotspot_stats_to = min($hotspot_list_page * $hotspot_list_per_page, $hotspot_stats_total);

        $ui->assign('hotspot_stats_total', $hotspot_stats_total);
        $ui->assign('hotspot_stats_enabled', $hotspot_stats_enabled);
        $ui->assign('hotspot_stats_disabled', $hotspot_stats_disabled);
        $ui->assign('hotspot_stats_expired_package', $hotspot_stats_expired_package);
        $ui->assign('hotspot_stats_from', $hotspot_stats_from);
        $ui->assign('hotspot_stats_to', $hotspot_stats_to);

        $query = $hotspotListBase();
        $applyHotspotListFilters($query, true);
        $d = Paginator::findMany($query, ['name' => $name], $hotspot_list_per_page, $append_url);
        $ui->assign('d', $d);
    ...
    case 'edit':
        ...
        if ($d) {
            ...
            $ui->assign('b', $b);
            $ui->assign('devices', Package::servicePlanDeviceOptions($DEVICE_PATH, $d['device']));
            //select expired plan
            if ($d['is_radius']) {
    ...
    case 'add-post':
        ...
        $routers = _post('routers');//selecting all routers
        if (is_array($routers)) {
    if (in_array('all', $routers)) {
        $routers = ['all'];
    }
    $routers = implode(',', $routers);
        ...
        if (!Package::servicePlanDeviceSelectionAllowed($device, null)) {
            $msg .= Lang::T('Invalid device selected') . '<br>';
        }
    ...
    $routers = implode(',', $routers);
        ...
        if ($d && !Package::servicePlanDeviceSelectionAllowed($device, $old['device'])) {
            $msg .= Lang::T('Invalid device selected') . '<br>';
        }
    ...
    case 'pppoe':
        ...
        $pppoeListBase = function () {
            return ORM::for_table('tbl_bandwidth')
                ->left_outer_join('tbl_plans', array('tbl_bandwidth.id', '=', 'tbl_plans.id_bw'))
                ->where('tbl_plans.type', 'PPPOE');
        };

        $applyPppoeListFilters = function ($q, $withStatusFilter) use ($type1, $type2, $type3, $bandwidth, $valid, $router, $device, $name, $status) {
            if (!empty($type1)) {
                $q->where('tbl_plans.prepaid', $type1);
            }
            if (!empty($type2)) {
            ...
            if (!empty($router)) {
                if ($router == 'radius') {
                    ...
                    $q->where('tbl_plans.routers', $router);
                }
            }
            if (!empty($device)) {
                $q->where('tbl_plans.device', $device);
        ...
        $qTotal = $pppoeListBase();
        $applyPppoeListFilters($qTotal, true);
        $pppoe_stats_total = $qTotal->count();

        $qEnabled = $pppoeListBase();
        $applyPppoeListFilters($qEnabled, false);
        $qEnabled->where('tbl_plans.enabled', '1');
        $pppoe_stats_enabled = $qEnabled->count();

        $qDisabled = $pppoeListBase();
        $applyPppoeListFilters($qDisabled, false);
        $qDisabled->where('tbl_plans.enabled', '0');
        $pppoe_stats_disabled = $qDisabled->count();

        $qExpiredPlan = $pppoeListBase();
        $applyPppoeListFilters($qExpiredPlan, false);
        $qExpiredPlan->where_raw('tbl_plans.plan_expired IS NOT NULL AND tbl_plans.plan_expired > 0');
        $pppoe_stats_expired_plan = $qExpiredPlan->count();

        $pppoe_list_per_page = 20;
        $pppoe_list_page = (int) _get('p', 1);
        if ($pppoe_list_page < 1) {
            $pppoe_list_page = 1;
        }
        $pppoe_stats_from = $pppoe_stats_total > 0 ? (($pppoe_list_page - 1) * $pppoe_list_per_page + 1) : 0;
        $pppoe_stats_to = min($pppoe_list_page * $pppoe_list_per_page, $pppoe_stats_total);

        $ui->assign('pppoe_stats_total', $pppoe_stats_total);
        $ui->assign('pppoe_stats_enabled', $pppoe_stats_enabled);
        $ui->assign('pppoe_stats_disabled', $pppoe_stats_disabled);
        $ui->assign('pppoe_stats_expired_plan', $pppoe_stats_expired_plan);
        $ui->assign('pppoe_stats_from', $pppoe_stats_from);
        $ui->assign('pppoe_stats_to', $pppoe_stats_to);

        $query = $pppoeListBase();
        $applyPppoeListFilters($query, true);
        $d = Paginator::findMany($query, ['name' => $name], $pppoe_list_per_page, $append_url);
