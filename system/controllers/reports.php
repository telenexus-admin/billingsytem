...
switch ($action) {
    case 'today':
        ...
        $today_transactions = ORM::for_table('tbl_transactions')
            ->where('recharged_on', $today)
            ->order_by_desc('recharged_time')
            ->find_array();
        
        // Calculate totals
        $today_count = count($today_transactions);
        $today_revenue = 0;
        $today_hotspot = 0;
        $today_pppoe = 0;
        ...
        foreach ($today_transactions as $transaction) {
            $today_revenue += floatval($transaction['price']);
            if ($transaction['type'] == 'Hotspot') {
                $today_hotspot++;
            } elseif ($transaction['type'] == 'PPPOE') {
                $today_pppoe++;
            }
        }
        
        // Get counts from database to ensure accuracy
        $db_hotspot = ORM::for_table('tbl_transactions')
            ->where('recharged_on', $today)
            ->where('type', 'Hotspot')
            ->count();
        
        $db_pppoe = ORM::for_table('tbl_transactions')
            ->where('recharged_on', $today)
            ->where('type', 'PPPOE')
            ->count();
        
        $db_revenue = ORM::for_table('tbl_transactions')
            ->where('recharged_on', $today)
            ->sum('price');
        
        // Use database values if they exist
        if ($db_hotspot > 0) $today_hotspot = $db_hotspot;
        if ($db_pppoe > 0) $today_pppoe = $db_pppoe;
        if ($db_revenue > 0) $today_revenue = floatval($db_revenue);
        
        // Assign variables to template
        $ui->assign('today_transactions', $today_transactions);
        $ui->assign('today_count', $today_count);
        $ui->assign('today_revenue', $today_revenue);
        $ui->assign('today_hotspot', $today_hotspot);
        $ui->assign('today_pppoe', $today_pppoe);
        $ui->assign('today_date', $today);
        
        // Display the template
        $ui->display('admin/reports/today.tpl');
        break;
