...
switch ($action) {
    ...
    case 'activation-post':
        ...
        // Fallback for numeric-only vouchers (BINARY collation issue)
        if (!$v1 && preg_match('/^[0-9]+$/', $code)) {
            $v1 = ORM::for_table('tbl_voucher')->where('code', $code)->where('status', 0)->find_one();
        }
        
        run_hook('customer_activate_voucher'); #HOOK
        if ($v1) {
            if (Package::rechargeUser($user['id'], $v1['routers'], $v1['id_plan'], "Voucher", $code, '', $code)) {
                $v1->status = "1";
