...
} else if ($action == 'ajax_online_users') {
    // AJAX endpoint for getting online users data
    header('Content-Type: application/json');
    header('Cache-Control: no-cache, must-revalidate');
    header('Expires: Mon, 26 Jul 1997 05:00:00 GMT');
    try {
        require_once $WIDGET_PATH . '/top_widget.php';
        $data = top_widget::ajaxGetOnlineUsers();
        
        // Use the offline count from the widget as inactive count
        $data['inactive'] = $data['offline'];
