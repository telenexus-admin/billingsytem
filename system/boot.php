...
try {
    ...
    if (empty($ui)) {
        die('<div style="font-family:monospace;padding:30px;background:#fff3cd;border:2px solid #f59e0b;margin:40px auto;max-width:800px;border-radius:8px;">'
            . '<h2 style="color:#b45309;">&#9888; Database Error</h2>'
            . '<p><strong>Message:</strong> ' . htmlspecialchars($e->getMessage()) . '</p>'
            . '<p style="color:#555;">Check that MySQL is running and the credentials in <code>config.php</code> are correct, then ensure all database tables are imported.</p>'
            . '</div>');
    }
    ...
    $ui->assign("error_title", "AllxSys System Error");
    $ui->display('admin/error.tpl');
    die();
} catch (Exception $e) {
    if (empty($ui)) {
        die('<div style="font-family:monospace;padding:30px;background:#fff3cd;border:2px solid #f59e0b;margin:40px auto;max-width:800px;border-radius:8px;">'
            . '<h2 style="color:#b45309;">&#9888; Startup Error</h2>'
            . '<p><strong>Message:</strong> ' . htmlspecialchars($e->getMessage()) . '</p>'
            . '</div>');
    }
...
try {
    ...
    if (file_exists($sys_render)) {
        ...
        $menu_slot_set = array_flip($menu_template_slots);
        foreach ($menu_template_slots as $slot) {
            $ui->assign('_MENU_' . $slot, $menus[$slot] ?? '');
        }
        foreach ($menus as $k => $v) {
            if (!isset($menu_slot_set[$k])) {
    ...
    $ui->assign("error_message", $e->getMessage() . '<br><pre>' . $e->getTraceAsString() . '</pre>');
    $ui->assign("error_title", "Antiqua System Error");
    $ui->display('admin/error.tpl');
    die();
} catch (Exception $e) {
    Message::sendTelegram(
