...
class Package
{
        ...
        if ($b) {
            ...
            if ($_app_stage != 'Demo') {
                try {
                    if (file_exists($dvc)) {
                        ...
                        $deviceName = $p['device'];
                        $dev = new $deviceName();
                        if ($deviceName === 'Radius') {
                            $dev->add_customer($c, $p, $date_exp . ' ' . $time);
                        } else {
                            $dev->add_customer($c, $p);
                        }
                    } else {
            ...
            try {
                ...
                if (strpos($e->getMessage(), 'Duplicate entry') !== false) {
                    _log("Duplicate transaction blocked: $inv (User: {$c['username']}, Plan: {$p['name_plan']})");
                    // Continue - user still gets activated
                } else {
            ...
            $t->user_id = $id_customer;
            $t->plan_name = $p['name_plan'];
            // Always set price to 0 for any voucher transaction
            if (strtolower($gateway) === 'voucher') {
                $t->price = 0;
            } else {
                if ($p['validity_unit'] == 'Period') {
    ...
    /**
     * Device dropdown for Hotspot / PPPOE plans: hide billing-only / VPN drivers.
     * If $currentDevice is set (edit), keep it selectable for legacy rows.
     */
    public static function servicePlanDeviceOptions($devicePath, $currentDevice = null)
    {
        $exclude = ['Dummy', 'MikrotikVpn'];
        $devices = [];
        if (!is_dir($devicePath)) {
            return $devices;
        }
        foreach (scandir($devicePath) as $file) {
        ...
        if ($currentDevice !== null && $currentDevice !== '' && in_array($currentDevice, $exclude, true) && !in_array($currentDevice, $devices, true)) {
    ...
    /** Hotspot / PPPOE: blocked devices only when unchanged on edit ($previousDevice === $device). */
    public static function servicePlanDeviceSelectionAllowed($device, $previousDevice = null)
    {
        $exclude = ['Dummy', 'MikrotikVpn'];
        if (!in_array($device, $exclude, true)) {
            return true;
        }
        return $previousDevice !== null && $device === $previousDevice;
    }
    ...
    public static function getDevice($plan)
    {
        ...
        // Radius plans must use the Radius DB driver even if device was left as Mikrotik* from legacy data
        if ($plan['is_radius'] == 1) {
            if ($plan['device'] === 'RadiusRest') {
                return $DEVICE_PATH . DIRECTORY_SEPARATOR . 'RadiusRest.php';
            }
            if ($plan['device'] !== 'Radius') {
                $plan->device = 'Radius';
                $plan->save();
            }
