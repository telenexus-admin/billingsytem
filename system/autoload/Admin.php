...
class Admin
{
    ...
    public static function getID()
    {
        ...
        $enable_session_timeout = (int) ($config['enable_session_timeout'] ?? 0) === 1;
        $session_timeout_duration = !empty($config['session_timeout_duration'])
            ? max(1, (int) $config['session_timeout_duration']) * 60
            : (int) (60 * 60);
        if ($isApi) {
        ...
        if ($enable_session_timeout && !empty($_SESSION['aid']) && !empty($_SESSION['aid_expiration'])) {
            if ($_SESSION['aid_expiration'] > time()) {
                ...
                if (empty($_COOKIE['aid'])) {
                    self::removeCookie();
                    _alert(Lang::T('Token has expired. Please log in again.') . '.' . $aid, 'danger', "admin");
                    return 0;
                }
                $isValid = self::validateToken($aid, $_COOKIE['aid']);
                if (!$isValid) {
                    self::removeCookie();
    ...
    public static function setCookie($aid)
    {
        ...
        $enable_session_timeout = (int) ($config['enable_session_timeout'] ?? 0) === 1;
        $durMin = isset($config['session_timeout_duration']) && $config['session_timeout_duration'] !== ''
            ? (int) $config['session_timeout_duration']
            : 60;
        $session_timeout_duration = $durMin > 0 ? $durMin * 60 : (int) (60 * 60);
    ...
    public static function removeCookie()
    {
        ...
        // Always clear the session token even if the cookie is already missing.
        unset($_SESSION['aid'], $_SESSION['aid_expiration']);
    }
