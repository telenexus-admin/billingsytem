...
class Mikrotik
{
    ...
    public static function listFiles($client)
    {
        global $_app_stage;
        if ($_app_stage == 'demo') {
            return [];
        }
        $req = new RouterOS\Request('/file/print');
        ...
        foreach ($responses as $response) {
            if ($response->getType() === RouterOS\Response::TYPE_DATA) {
                $files[] = [
                    ...
                    'size' => (string) $response->getProperty('size'),
                ];
            }
        }
        return $files;
    ...
    public static function getHotspotProfiles($client)
    {
        global $_app_stage;
        if ($_app_stage == 'demo') {
            return [];
        }
        $req = new RouterOS\Request('/ip/hotspot/profile/print');
        $responses = $client->sendSync($req);
        $profiles = [];
        foreach ($responses as $response) {
            if ($response->getType() === RouterOS\Response::TYPE_DATA) {
                $profiles[] = [
                    'name'           => (string) $response->getProperty('name'),
                    'html-directory' => (string) $response->getProperty('html-directory'),
                ];
            }
        }
        return $profiles;
    ...
    public static function fetchFile($client, $url, $dstPath)
    {
        global $_app_stage;
        if ($_app_stage == 'demo') {
            return;
        }
        $req = new RouterOS\Request('/tool/fetch');
        $client->sendSync(
            $req
                ->setArgument('url', $url)
                ->setArgument('dst-path', $dstPath)
                ->setArgument('mode', 'http')
        );
    }

    public static function setHotspotHtmlDir($client, $profileName, $htmlDir)
    {
        global $_app_stage;
        if ($_app_stage == 'demo') {
            return;
        ...
        $printRequest = new RouterOS\Request(
            '/ip hotspot profile print .proplist=.id',
            RouterOS\Query::where('name', $profileName)
        );
        $profileId = $client->sendSync($printRequest)->getProperty('.id');

        if (empty($profileId)) {
            // Fall back to first available profile
            $allReq   = new RouterOS\Request('/ip/hotspot/profile/print');
            $allReq->setArgument('.proplist', '.id');
            $profileId = $client->sendSync($allReq)->getProperty('.id');
        }
        ...
        if (!empty($profileId)) {
            ...
            $client->sendSync(
                $setRequest
                    ->setArgument('numbers', $profileId)
                    ->setArgument('html-directory', $htmlDir)
            );
        }
    }
}
