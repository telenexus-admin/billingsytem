...
function radon_users()
{
    ...
    $useron = ORM::for_table('radacct', 'radius')
    ...
    if (isset($_POST['kill'])) {
        ...
        $nasIp = _post('nas_ip');
        $nas = ORM::for_table('nas', 'radius')->where_like('nasname', "%$nasIp%")->find_one();

        if ($nas) {
            $sharedsecret = $nas['secret'];
            $userOnline = ORM::for_table('radacct', 'radius')
...
function radon_users_cleandb()
{
	...
	$admin = Admin::_info();
	try {
		ORM::get_db('radius')->exec('TRUNCATE TABLE `radacct`');
