...
function BankStkPush_payment_notification()
{
    ...
    if ($response_code == "0") {
        ...
        // Check for duplicate transaction by Mpesa code (invoice) with additional safety
        $existingTransaction = ORM::for_table('tbl_transactions')
            ->where('invoice', $mpesa_code)
            ->find_one();

        if ($existingTransaction) {
            // Duplicate found, do not record again
            _log('Duplicate Mpesa transaction detected in tbl_transactions: ' . $mpesa_code);
            Message::sendTelegram("Duplicate Mpesa transaction blocked: " . $mpesa_code);
            exit();
        }

        // Additional check: Look for duplicate gateway transactions with same Mpesa code
        $existingGatewayTransaction = ORM::for_table('tbl_payment_gateway')
            ->where('gateway_trx_id', $mpesa_code)
            ->where('status', 2) // Only check successfully paid transactions
            ->where_not_equal('id', $PaymentGatewayRecord->id) // Exclude current record
            ->find_one();

        if ($existingGatewayTransaction) {
            // Duplicate found in gateway table
            _log('Duplicate Mpesa transaction detected in tbl_payment_gateway: ' . $mpesa_code);
            Message::sendTelegram("Duplicate Mpesa gateway transaction blocked: " . $mpesa_code);
            exit();
        }
